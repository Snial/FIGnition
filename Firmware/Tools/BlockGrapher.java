...
public class BlockGrapher extends JFrame {
    ...
    public int h=0,w=0;
    public int targetH=0,targetW=0;
	byte[] rawImg;
	int rawImgLen=0;
	int inverse=0xff;
	...
	/**
	 * Block graphics are organized in groups of 2x2 pixels.
	 * Thus bit 0=top, right, bit 1=top, left, bit 2=bottom, right
	 * But if bottom left is white then we need to xor the
	 * values of the remaining bits with 0x87. But for the moment
	 * we'll ignore that stage and just treat it as 4 normal pixels.
	 * 
	 **/
	...
	public byte[] convert(String srcFile, boolean dither) {
        ...
        /* The image is assumed to be in monochrome,
           so red=green=blue, but it does need to be
           inverted.
         */
        int rows=(targetH+1)&~1;
        int cols=(targetW+1)&~1;
        int midX=w/(2*targetW);
        int midY=h/(2*targetH);
        byte[] bwbitmap=new byte[rows*cols/4];
        for (int i=0; i<rows; i++){
        	int y=i*h/rows+midY;
            for (int j=0; j<cols; j++){
            	int x=j*w/cols+midX;
                FIGplot(bwbitmap,i,j,cols,data[x*w+y]>>16&1);
            }
        }		
	...
	/**
	 * A FIGgy Block graphic Image consists of up to 24 lines
	 * of 25 characters, each containing
	 * raw block character graphics converted and offset.
	 *
	 * ,img ............Imgdata<32>
	 *
	 * Each character contains 4-bits of data, corresponding to
	 * the raw graphics characters: 32,17..23 and 160,145 to 151
	 * offset by 32 to put them all in a printable range.
	 * An image block ends in a space.
	 *
	 * The maximum per ,blks will be 255 bytes.
	 *
	 *
	 * ,blks ....moreImgData
	 *
	 ...
	 * iiiiiiiii ,img ....:...10
	 *
	 * An entire image is therefore: 2 blocks.
	 *
	 * ....:....1....:....2....5
	 * ,img[ ....:....1....:...9
	 * ....:....1....:....2....5
	...
	/**
	 * Here ix is an offset in block graphics order and we want to convert it
	 * to its raster position. 
	 **/
 ...
 **/
	...
	public static void main(String[] args){
		if(args.length<3)
			System.out.println("usage: BlockGrapher -t targetHeight targetWidth srcFile dstHexFileBase cDataTitle"); 
		else {
			...
			if(args[argc]=="-t") {
				frame.targetH=Integer.parseInt(args[argc+1]);
				frame.targetW=Integer.parseInt(args[argc+2]);
				argc+=3;
			}
